...
export function AlertCard({ alert }: AlertCardProps) {
  ...
  return (
    <article className="border-b border-border/80 p-3.5 last:border-0">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-2">
            <p className="min-w-0 text-sm font-semibold leading-5 text-slate-100">{alert.title}</p>
          ...
          <p className="mt-2 text-xs font-medium uppercase tracking-normal text-slate-500">{alert.platform}</p>
        </div>
      </div>
    </article>
  )
